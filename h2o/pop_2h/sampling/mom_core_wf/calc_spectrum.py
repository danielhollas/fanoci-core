...
def read_cmd():
   ...
   parser.add_option('-s','--sigma',dest='sigma', type='float', default = -1, help='Parameter for Gaussian broadening.')
...
#  --smooth (perform runnning average?)
   ...
   parser.add_option('','--arbunits', dest='arb_units', action="store_true",default=False,
         help = 'Arbitrary units, instead of trans. dip. moments')
...
class Spectrum(object):
   ...
   def __init__(self, nsample, deltaE, notrans, arb_units, subset, cycles, ncores, jobs):
      self.trans = []
      self.intensity = []
      self.exc = []
      self.energies = []
   ...
   def arbitrary_units(self): 
      self.intensity = [ 0.0 for i in range(int(self.maxe/self.de))]
      for i in range(len(self.trans)):
         index = int (round( (self.maxe-self.exc[i]) / self.de ) )
         intensity = self.trans[i] / self.de / self.nsample
   ...
   def read_data(self, infile):
      ...
      with open(infile, "r") as f:
         for line in f:
            if self.arb_units:
               l = line.split()
               self.exc.append(float(l[0]))
               self.trans.append(float(l[1]))
            elif (i % 3 == 1 and self.subset > 0 and self.notrans == False) or (i % 2 == 1 and self.subset == 0 and self.notrans == False):
               temp = line.split()
               try:
                  # assigning transition dipole moments as a tuple
                  self.trans.append( ( float(temp[0]), float(temp[1]), float(temp[2]) ) )
               except:
                  print("Error: Corrupted line %d in file %s" % (i+1, infile))
                  print("I expected 3 columns of transition dipole moments, but got:")
                  print(line)
                  #raise
                  sys.exit(1)
...
#      assert(len(self.exc)==len(self.trans))
   ...
   def writeoutall(self, infile, message=""):
      ...
      if self.arb_units or self.notrans:
      ...
      toprint = message
      for un in xunits:
         outfile="absspec.%s.%s.%d.cross.dat" % (name, un, self.nsample)
         toprint += "\n\tPrinting spectrum in units [%s, %s] to %s" % (un, yunits, outfile)
...
class SpectrumBroad(Spectrum):
   ...
   def __init__(self, nsample, deltaE, sigma, tau, notrans, arb_units, subset, cycles, ncores, jobs):
      self.trans = []
      self.intensity = []
      self.exc = []
      self.energies = []
   ...
   def arbitrary_units(self): 
      if self.sigma > 0:
         self.int_sigma = [ intensity / self.sigma / math.sqrt(2*PI) / self.nsample for intensity in self.trans]

      if self.tau > 0:
         self.int_tau = [ intensity * self.tau / 2 / PI / self.nsample for intensity in self.trans]
...
if options.tau > 0.0 or options.sigma >= 0:
   spectrum = SpectrumBroad(options.nsample, options.de, options.sigma, options.tau, options.notrans, options.arb_units, options.subset, options.cycles, options.ncores, options.jobs)
else:
   spectrum = Spectrum(options.nsample, options.de, options.notrans, options.arb_units, options.subset, options.cycles, options.ncores, options.jobs)

spectrum.read_data(infile)
if spectrum.subset > 0:
   spectrum.reduce_geoms(infile)
else:
   if options.notrans == True:
      spectrum.normalize()
   elif options.arb_units == True:
      spectrum.arbitrary_units()
   else:
