...
######## SETUP ##########
# Keeping "neutral", but we're calculating MOM!
# Needed to set scrdirs in calc.TERA.sh
name=h2o-aes-neutral    # name of the job
firstgeom=301             # first geometry, will skip (first-1)geometries
lastgeom=500              # last geometry, positive integer or 0 for all geometries up to the end of file
movie=geoms.xyz         # file with xyz geometries
program=TERA            # one of GAUSS, MOLPRO, QCHEM, ORCA
jobs=4                  # determines number of jobs to submit
                        # the calculations will be distributed accordingly
nproc=1
submit="qsub -V -cwd -q nq-gpu@n26 -l num_gpu=1 -pe shm $nproc " # comment this line if you do not want to submit jobs automatically
